...
public:
    ...
    auto insert(const domain::player& player) -> void
    {
        ...
        if (player.get_weapon())
        {
            auto update_query = fmt::format(
                "UPDATE players SET weapon_name = \"{}\" WHERE "
                "login = \"{}\"; ",
        ...
        {
            auto update_query = fmt::format(
                "UPDATE players SET armor_name = \"{}\" WHERE "
                "login = \"{}\"; ",
                player.get_weapon()->get_name(),
                player.get_name()
            );
    ...
    auto remove(const domain::player& player) -> void
    {
        const auto delete_player_query = fmt::format(
            "DELETE FROM players WHERE login = \"{}\";",
            player.get_name()
        );
        ...
        -> domain::player::pointer
    {
        auto name = std::find_if(
                        ...
                        [](const auto& p)
                        {
                            return p.first == "login";
                        }
        )->second;
        auto pos_x = std::find_if(
                         std::begin(record),
                         std::end(record),
                         [](const auto& p)
                         {
                             return p.first == "pos_x";
                         }
        )->second;
        auto pos_y = std::find_if(
                         std::begin(record),
                         std::end(record),
                         [](const auto& p)
                         {
                             return p.first == "pos_y";
                         }
        )->second;
        ...
        auto money = std::find_if(
                         std::begin(record),
                         std::end(record),
                         [](const auto& p)
                         {
                             return p.first == "money";
                         }
        )->second;
        auto backpack_capacity = std::find_if(
                                     std::begin(record),
                                     std::end(record),
                                     [](const auto& p)
                                     {

...
public:
    ...
    auto insert(const domain::weapon& weapon) -> void
    {
        insert_impl(weapon);
        const auto items_query = fmt::format(
            "UPDATE item SET attack = {} WHERE name = \"{}\"",
            weapon.get_attack(),
            weapon.get_name()
        );
        db_.run_query(items_query);
    ...
    auto insert(const domain::armor& armor) -> void
    {
        insert_impl(armor);
        const auto items_query = fmt::format(
            "UPDATE items SET defense = {} WHERE name = \"{}\"",
            armor.get_defense(),
            armor.get_name()
        );
        db_.run_query(items_query);
    ...
    auto insert(const domain::health_potion& potion) -> void
    {
        insert_impl(potion);
        const auto items_query = fmt::format(
            "UPDATE item SET power = {} WHERE name = \"{}\"",
            potion.get_power(),
            potion.get_name()
        );
        db_.run_query(items_query);
    ...
    {
        update_impl(weapon);
        db_.run_query(fmt::format(
            "UPDATE items SET attack = {} WHERE name = \"{}\";",
            weapon.get_attack(),
            weapon.get_name()
        ));
    }
    ...
    auto update(const domain::health_potion& potion) -> void
    {
        update_impl(potion);
        db_.run_query(fmt::format(
            "UPDATE items SET power = {} WHERE name = \"{}\"",
            potion.get_power(),
            potion.get_name()
        ));
    }

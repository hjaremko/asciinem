...
public:
        ...
        for (const auto& m : monsters)
        {
            if (is_near(player, m))
            {
                spdlog::warn(fmt::format(
                    "Player {} got in a fight with monster {}",
                    player->get_name(),
                    m->get_name()
                ));
                ...
                else
                {
                    ...
                    current_state_.set_notice(fmt::format(
                        "Player {} has lost a fight! Respawning...",
                        login
                    ));
                    player->reset();
                }
    ...
    void tick()
    {
        ...
        if (ticks_ % 1'000 == 0 && current_state_.get_monsters().size() < 3)
        {
...
private:
    ...
    static auto is_near(
        const entity::pointer& first,
        const entity::pointer& second
    ) -> bool
    {
        auto dist_x = 0;

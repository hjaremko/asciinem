...
player::player(const std::string& name)
    : entity(name, "\\o>", {28, 26}, 1)
    , exp_(0)
    , money_(10.)
    , // NOLINT
    backpack_({})
    , backpack_capacity_(5) // NOLINT
{
}
...
player::player(
    ...
    : entity(name, "\\o>", position, level)
    , exp_(exp)
    , money_(money(amount))
    , backpack_(std::move(backpack))
    , backpack_capacity_(
          std::min(backpack_capacity, static_cast<unsigned int>(10))
      )
    , // NOLINT
...
player::player(
    ...
    : entity(name, "\\o>", position, health, level)
    , exp_(exp)
    , money_(money(amount))
    , backpack_(std::move(backpack))
    , backpack_capacity_(
